#align(center)[
  #text(size: 30pt)[*Reflow*] \
  #text(size: 25pt)[User guide]
]

= Description

Reflow implements a content layout algorithm to provide text threading
(when text from one box spills into a different box if it overflows)
and image wrap-around.

#import "../src/lib.typ" as reflow
...
The main function provided is `reflow.reflow`, which takes as input some content,
...
Containers are created by the function `reflow.container`, and everything
...
#table(columns: (1fr, 1fr), stroke: none)[
  `reflow.reflow` is contextual, so the invocation needs to be wrapped in a
...
#table(columns: (1fr, 1fr), stroke: none)[
  `reflow.reflow` can handle as many obstacles as you provide
