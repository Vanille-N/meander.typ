#import "/src/lib.typ" as meander

#let cutout-offset = 5cm
#let cutout-width = 10cm

#let norm(x,y) = calc.pow(2 * x - 1, 2) + calc.pow(2 * y - 1, 2)

#let handler-right(overflow) = {
  meander.reflow(
    overflow: panic,
    {
      import meander: *
      placed(
        center, dy: cutout-offset,
        boundary: contour.grid(div: 20, (x,y) => norm(x,y) <= 1),
        box(width: cutout-width, height: cutout-width),
      )
      container(width: 48%, align: right)
      overflow.structured
    }
  )
}

#let handler-center(overflow) = {
  meander.reflow(
    debug: true,
    overflow: handler-right,
    {
      import meander: *
      placed(
        center, dy: cutout-offset,
        boundary: contour.grid(div: 20, (x,y) => norm(x,y) >= 0.7),
        box(width: cutout-width, height: cutout-width),
      )
      container(align: center, dy: cutout-offset, width: cutout-width, height: cutout-width)
      overflow.structured
    }
  )
}

#meander.reflow(
  debug: false,
  overflow: handler-center,
  {
    import meander: *
    placed(
      center, dy: cutout-offset,
      boundary: contour.grid(div: 20, (x,y) => norm(x,y) <= 1),
      box(width: cutout-width, height: cutout-width),
    )
    container(width: 48%)
    content[
      #set par(justify: true)
      #lorem(570)
    ]
  }
)
