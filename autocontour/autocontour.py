...
def parse_args():
    ...
    if image_path is None:
        ...
        sys.exit(1)
    return Args(path=image_path, width=width, height=height, threshold=threshold, commit=commit)
...
class Pixel:
    ...
    @staticmethod
    def from_bytes(bb):
        return Pixel(RGB255.from_bytes(bb))
...
class Image:
    ...
    def from_bytes_rgb255(bb, *, width, height):
        return Image([[Pixel.from_bytes(body[p:p+3]) for j in range(width) for p in [(i * width + j) * 3]] for i in range(height)])
